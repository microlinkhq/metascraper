...
/**
 * Scrape metadata using given attributes.
 *
 * @param {String} html
 * @param {String} url
 * @param {Window} window
...
function scrape ({ html, url, window, rules }) {
  if (window) {
    return scrapeMetadata(window.document.documentElement.outerHTML, window.location.href, rules)
  }

  return html
    ? scrapeMetadata(html, url, rules)
    : url
        ? scrapeUrl(url, rules)
        : Promise.reject(new Error(`Not enough attributes for scraping.
            At least one of window, url and/or html should be provided.`))
}
